...
- 用 `configs/hrv/core.yaml`、`configs/sleep_edf/default.yaml`、`configs/mit_bih_psg/default.yaml` 驱动可复现 pipeline。
...
- 增加 `configs/mit_bih_psg/default.yaml`。
...
| 目的 | 文件 |
| --- | --- |
| ECG/HRV core report | `reports/hrv/core_report.md` |
| Frozen HRV release bundle | `releases/hrv-core-v0.1.0/` |
| Sleep-EDF next work plan | `reports/sleep_edf/next_work_plan.md` |
...
| Respiratory dataset candidates | `reports/project/respiratory_dataset_candidates.md` |
...
uv run python -m physio_signal_lab.cli run-sleep-edf-pilot-benchmark --config configs/sleep_edf/default.yaml --records SC4001,SC4011,SC4021,SC4031,SC4041,SC4051,SC4061,SC4071,SC4081,SC4091,SC4101,SC4111,SC4121,SC4131,SC4141,SC4151,SC4161,SC4171,SC4181,SC4191 --output-prefix twenty_record --include-yasa
