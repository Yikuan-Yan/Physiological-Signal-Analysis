...
2. `data/manifests/fantasia.csv`：记录数据集名称、版本、DOI、许可、访问日期、文件范围和 checksum；
...
6. `reports/hrv/core_report.md` 与可重建 figures；
...
建立 `data/manifests/fantasia.csv`，字段至少为：
...
5. 输出 `results/hrv/data_quality/fantasia_inventory.csv`；
...
python -m physio_signal_lab.cli run-hrv-core --config configs/hrv/core.yaml
python -m physio_signal_lab.cli build-report hrv-core
pytest -q
```

`reports/hrv/core_report.md` 必须包含：
...
| 条件 | 验收方式 |
|---|---|
| 数据与许可可追溯 | `data/manifests/fantasia.csv` 完整，DOI/version/license/access date 可查 |
